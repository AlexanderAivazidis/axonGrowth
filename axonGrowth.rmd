...
Based on the JackStraw procedure I select 6 PCs for further analysis:
...
```{r, include = TRUE, message = FALSE, warning = FALSE, cache = FALSE, fig.height = 8, fig.width = 16}
axonGrowth <- FindNeighbors(axonGrowth, dims = 1:6)
...
```{r, include = TRUE, message = FALSE, warning = FALSE, cache = FALSE, fig.height = 8, fig.width = 16}
axonGrowth <- RunUMAP(axonGrowth, dims = 1:6)
...
correlationScores = data.frame(matrix(0,6,7))
...
rownames(temp) = paste('PCA', 1:6, sep = ' ')

Heatmap(temp, cluster_rows = FALSE)
```

Now principal components 2 might show some relationship to our biological variable of interest, while PC1 is dominated by technical factors. Plotting the PC2 score vs. TotalNeuriteLength or LongestNeuriteLength does not show a very convincing relationship:
